...
# visit only 3 towns. Which distances, hence which towns, they will
# choose so that the sum of the distances is the biggest possible to
# please Mary and John?
#
# Example:
...
# integer >= 0), k (number of towns to visit, k >= 1) and ls (list
# of distances, all distances are positive or zero integers and this
# list has at least one element). The function returns the "best" sum
# ie the biggest possible sum of k distances less than or equal to the
# given limit t, if that sum exists, or otherwise
...
# in some languages this "list" is in fact a string
# (see the Sample Tests).
from itertools import combinations


# this one in solution was beautiful :
# def choose_best_sum(t, k, ls):
#     return max(
#       (sum(v) for v in combinations(ls,k) if sum(v)<=t),
#       default=None
#     )
...
def choose_best_sum(t, k, ls):
    distances = [sum(x) for x in combinations(ls, k) if sum(x) <= t]
    distances.sort(reverse=True)
    return None if len(distances) == 0 else distances[0]


class Test5BestTravel(TestCase):
